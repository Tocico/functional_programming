...
public class RewriteMe {
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category){
        return questions.stream().filter(e -> e.category.equals(category)).collect(Collectors.toList()).size();
    }
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
        return questions.stream().filter(e -> e.category.equals(category)).map(Question::getQuestionString).collect(Collectors.toList());
    }
    ...
    public List<String> getAllAnswerOptionsDistinct(){
      return questions.stream().map(Question::getAllAnswers).flatMap(l -> l.stream()).distinct().collect(Collectors.toList());
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        return questions.stream().map(Question::getAllAnswers).flatMap(l -> l.stream()).distinct().anyMatch(e -> e.equalsIgnoreCase(answerCandidate));
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return questions.stream().map(Question::getAllAnswers).flatMap(l -> l.stream()).filter(e -> e.equalsIgnoreCase(answerCandidate)).collect(Collectors.toList()).size();
    }
    ...
    {
        return questions.stream().filter(e-> e.category.equals(c)).map(Question::getAllAnswers).flatMap(e -> e.stream()).distinct().reduce((e1, e2) -> e1.length() > e2.length() ? e1 : e2).get();
    }
